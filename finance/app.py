...
def buy():
    if request.method == "GET":
        ...
        if request.form.get("symbol") == None:
            ...
            response=lookup(request.form.get("symbol"))
            trade= db.execute("INSERT INTO transactions (user_id,timestamp,symbol,price,quantity,total) VALUES (:userid,:time,:symbol,:price,:quantity,:total)",userid=session["user_id"],time=current_time,symbol=request.form.get("symbol"), price=response["price"],quantity=request.form.get("shares"),total=amount )
